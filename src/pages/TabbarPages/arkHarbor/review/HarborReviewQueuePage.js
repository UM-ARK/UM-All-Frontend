...
const ReviewDetailHeader = ({reviewable, isLoading}) => {
    const {theme} = useTheme();
    const {t} = useTranslation('harbor');
...
const HarborReviewQueuePage = ({navigation}) => {
    const {theme} = useTheme();
    const {t} = useTranslation('harbor');
    const {
        patchReviewCount,
    ...
    const [selectedReviewable, setSelectedReviewable] = useState(null);
    ...
    useEffect(() => {
        navigation?.setOptions?.({headerTitle: t('Harbor 審核')});
    }, [navigation, t]);
    ...
    const loadQueue = useCallback(
        ...
        [nextPage, patchReviewCount, status, user?.username],
    );
    ...
    useEffect(() => {
        if (status === 'signedIn' && !initialLoadStartedRef.current) {
            initialLoadStartedRef.current = true;
            loadQueue();
        }
    }, [loadQueue, sessionGeneration, status]);
    ...
    return (
        <View style={[styles.page, {backgroundColor: theme.bg_color}]}>
            ...
            <Modal
                ...
                <SafeAreaView
                    edges={['top', 'bottom']}
                    style={[styles.modalPage, {backgroundColor: theme.bg_color}]}>
                    <View
                        style={[
                            styles.modalHeader,
                            {
                        ...
                        <Text style={[styles.modalTitle, {color: theme.black.main}]}>
                            {t('審核案件')}
                        </Text>
                                ...
                                <View style={styles.reasonModalActions}>
                                    ...
                                    <Pressable
                                        ...
                                        <Text
                                            ...
                                            {getActionLabel(reasonAction) || t('確認')}
                                        </Text>
                                    </Pressable>
                                </View>
                            </View>
